...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("children() to get the children of DOM elements - breadcrumbs", () => {
    cy.get('.traversal-breadcrumb').children('.active').should('contain', 'Contact Us')
  });

  it("closest() to validate the closest ancestor DOM element", () => {
    //Locating an closest ancestor element 'ul' of the child '.traversal-badge'
    cy.get('.traversal-badge').closest('ul').should('have.class', 'list-group')  // in the assertion when we use classes we don't need to add "." (example: '.list-group')
  });

  it("eq() to retrieve a specific element based on index", () => {
    cy.get('.traversal-drinks-list > *').eq(2).should('contain', 'Milk')   // select milk - eq(2)
  });
  ...
  it("not() to remove DOM element(s) from the set of elements", () => {
    ...
    // cy.get('.traversal-button-states > button').not('.disabled') - meaning - get all elements that does not have a clas "disabled"
    // .should('not.have.class', 'disabled') - assertion - All elements that were fetched should not contains the class "disabled"
  });
